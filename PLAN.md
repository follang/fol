...
- `0.1` `pending` Add `fol-lower` to the workspace and root crate dependencies.
- `0.2` `pending` Add `fol-lower` public API shell with `LoweringResult`, `Lowerer`, and error surface.
- `0.3` `pending` Add lowering error kinds and diagnostics integration.
- `0.4` `pending` Add smoke tests proving a typed workspace can be handed to the lowerer.

### Phase 1. IR Model Foundation

- `1.1` `pending` Define lowering-owned ID types for packages, globals, routines, blocks, locals, instructions, and lowered types.
...
- `2.1` `pending` Create lowering session state over `TypedWorkspace`.
