...
- `3.1` `done` Lower builtin scalar types and routine signature types into lowering-owned type data.
- `3.2` `done` Lower aliases by erasing them to underlying runtime shapes while preserving debug metadata.
- `3.3` `done` Lower record type declarations into explicit field layouts.
...
- `6.2` `done` Lower entry construction into explicit tag/payload instructions.
...
- `9.2` `done` Add debug/snapshot output for lowered IR so tests and humans can inspect it deterministically.
- `9.3` `done` Add end-to-end CLI tests for successful lowering across local, std, and pkg graphs.
- `9.4` `done` Add end-to-end CLI tests for lowering failures and structured JSON diagnostics.
...
- `10.3` `done` Update relevant book/compiler docs to explain the role of lowering without promising backend details too early.
