...
impl Source {
    ...
    pub fn init_with_package(input: &str, source_type: SourceType, package_name: &str) -> Result<Vec<Self>, Box<dyn Glitch>> {
        source(input, source_type, package_name)
    }
...
impl FileStream {
    ...
    pub fn from_sources(mut sources: Vec<Source>) -> Result<Self, Box<dyn Glitch>> {
        ...
        for source in &mut sources {
            source.data = std::fs::read_to_string(&source.path)
...
impl CharacterProvider for FileStream {
    fn next_char(&mut self) -> Option<(char, Location)> {
        ...
        while self.current_source < self.sources.len() {
            let current_data = &self.sources[self.current_source].data;
            
            if self.position >= current_data.len() {
                // Move to next source
            ...
            if let Some(ch) = current_data.chars().nth(self.position) {
...
fn source(input: &str, source_type: SourceType, package_name: &str) -> Result<Vec<Source>, Box<dyn Glitch>> {
...
fn compute_namespace(file_path: &str, root_path: &str, package_name: &str) -> Result<String, Box<dyn Glitch>> {
