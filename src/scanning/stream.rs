...
impl STREAM {
    ...
    pub fn seek(&self) -> SCAN {
        if self.nth(2).key().is_space() { self.nth(3) } else { self.nth(2) }
    }
    ...
    pub fn report_bracket(&mut self, k: String, l: locate::LOCATION, e: &mut flaw::FLAW) {
        ...
        self.report(s, l, e, flaw::flaw_type::lexer(flaw::lexer::lexer_bracket_unmatch));
    }
    pub fn report_primitive_acccess(&mut self, k: String, l: locate::LOCATION, e: &mut flaw::FLAW) {
        let s = String::from("primitive type: ") + k.as_str().black().on_white().to_string().as_str() + " doesn't have fields to access ";
        self.report(s, l, e, flaw::flaw_type::lexer(flaw::lexer::lexer_primitive_access));
    }
    pub fn report_space_add(&mut self, k: String, l: locate::LOCATION, e: &mut flaw::FLAW) {
        ...
        self.report(s, l, e, flaw::flaw_type::lexer(flaw::lexer::lexer_space_add));
    }
