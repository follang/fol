...
impl fmt::Display for var_stat {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if let Some(a) = self.get_options().clone() {
            for e in a.iter() {
                opts.push(e.clone().to_string())
            }
        }
        let id: String = self.get_ident().clone().get().to_string();
        let mut body = String::new();
        if let Some(a) = self.get_body().clone() {
            body = " = ".to_string() + a.to_string().as_str();
        }
        write!(f, "{:?} {}{}{}\t{};", opts, id, typ, body, base)
    }
}
