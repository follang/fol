...
impl forest {
    pub fn init(&mut self, lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) {
        if !flaw.list().is_empty() {
            return;
        }
        let f = self::new();
        while lex.not_empty() {
            if let Err(e) = self.parse_tree(lex, flaw) {
                lex.to_endline(flaw);
