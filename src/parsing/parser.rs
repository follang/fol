...
fn help_assign_identifiers(list: &mut Vec<ID<String>>, lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> Con<()> {
    //identifier
...
fn help_assign_retypes(types: &mut Vec<tree>, lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> Con<()> {
...
fn help_assign_definition(opts: &mut Vec<assign_opts>, lex: &mut lexer::BAG, flaw: &mut flaw::FLAW,
...
fn help_assign_options(v: &mut Vec<assign_opts>, lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> Con<()> {
    if matches!(lex.curr().key(), KEYWORD::option(_)) {
        let el;
        match lex.curr().key() {
...
fn parse_type_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    match lex.curr().key() {
