...
fn retypes_int_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Int)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_flt_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Flt)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_chr_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Chr)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_bol_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Bol)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_arr_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Arr)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_vec_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Vec)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_seq_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Seq)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_mat_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Mat)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_set_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Set)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_map_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Map)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_axi_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Axi)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_tab_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Tab)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_str_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Str)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_num_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Num)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_ptr_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Ptr)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_err_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Err)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_opt_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Opt)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_nev_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Nev)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_uni_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Uni)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_any_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Any)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_non_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Non)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_nil_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Nil)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_rec_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Rec)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_ent_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Ent)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_blu_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Blu)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_std_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Std)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_loc_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Loc)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_url_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Url)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_blk_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Blk)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_rut_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Rut)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_pat_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Pat)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
...
fn retypes_gen_stat(lex: &mut lexer::BAG, flaw: &mut flaw::FLAW) -> tree {
    let typ = tree::new(
        lex.curr().loc().clone(),
        tree_type::stat(stat_type::Retype(retype_stat::Gen)),
    );
    lex.bump();
    temp_go_end_type(lex);
    typ
