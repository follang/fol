...
pub enum FolType {
    // Ordinal types
    Int { size: Option<IntSize>, signed: bool },
    Float { size: Option<FloatSize> },
    Char { encoding: CharEncoding },
    Bool,
    
    // Container types
    Array { element_type: Box<FolType>, size: Option<usize> },
    Vector { element_type: Box<FolType> },
    Sequence { element_type: Box<FolType> },
    Matrix { element_type: Box<FolType>, dimensions: Vec<usize> },
    Set { types: Vec<FolType> }, // Tuple-like heterogeneous set
    Map { key_type: Box<FolType>, value_type: Box<FolType> },
    
    // Complex types
    Record { fields: HashMap<String, FolType> },
    Entry { variants: HashMap<String, Option<FolType>> }, // Enum-like
    
    // Special types
    Optional { inner: Box<FolType> }, // opt[T]
    Multiple { types: Vec<FolType> }, // mul[T1, T2, ...]
    Any,
    Pointer { target: Box<FolType> },
    Error { inner: Option<Box<FolType>> },
    None,
    
    // Function types
    Function { params: Vec<FolType>, return_type: Box<FolType> },
    
    // Generic and module types
    Generic { name: String, constraints: Vec<FolType> },
    Module { name: String },
    Block { name: String },
...
pub enum TypeDefinition {
    Record { fields: HashMap<String, FolType> },
    Entry { variants: HashMap<String, Option<FolType>> },
    Alias { target: FolType },
}
...
pub enum WhenCase {
    /// case(condition) { body }
    Case { condition: AstNode, body: Vec<AstNode> },
    ...
    Of { type_match: FolType, body: Vec<AstNode> },
    /// on(channel) { body } - for channel matching
    On { channel: AstNode, body: Vec<AstNode> },
}
...
pub enum LoopCondition {
    ...
    Iteration { var: String, iterable: Box<AstNode>, condition: Option<Box<AstNode>> },
}
...
impl AstNode {
    ...
    pub fn get_type(&self) -> Option<FolType> {
        match self {
            ...
            AstNode::Literal(Literal::String(_)) => Some(FolType::Named { name: "str".to_string() }),
            AstNode::Literal(Literal::Character(_)) => Some(FolType::Char { encoding: CharEncoding::Utf8 }),
            ...
            AstNode::UnaryOp { op, operand } => {
                match op {
                    ...
                    UnaryOperator::Ref => {
                        operand.get_type().map(|t| FolType::Pointer { target: Box::new(t) })
                    },
                    UnaryOperator::Deref => {
    ...
    pub fn children(&self) -> Vec<&AstNode> {
        match self {
            ...
            AstNode::When { expr, cases, default } => {
                ...
                for case in cases {
                    match case {
                        WhenCase::Case { condition, body } | WhenCase::Is { value: condition, body } |
                        WhenCase::In { range: condition, body } | WhenCase::Has { member: condition, body } |
                        WhenCase::On { channel: condition, body } => {
            ...
            AstNode::Loop { condition, body } => {
                ...
                match condition.as_ref() {
                    ...
                    LoopCondition::Iteration { iterable, condition: iter_cond, .. } => {
...
pub trait AstVisitor {
    ...
    fn visit_var_decl(&mut self, _options: &[VarOption], _name: &str, _type_hint: &Option<FolType>, _value: &Option<Box<AstNode>>) {}
    fn visit_fun_decl(&mut self, _options: &[FunOption], _name: &str, _params: &[Parameter], _return_type: &Option<FolType>, _body: &[AstNode]) {}
    fn visit_pro_decl(&mut self, _options: &[FunOption], _name: &str, _params: &[Parameter], _return_type: &Option<FolType>, _body: &[AstNode]) {}
    fn visit_type_decl(&mut self, _options: &[TypeOption], _name: &str, _type_def: &TypeDefinition) {}
    ...
    fn visit_when(&mut self, _expr: &AstNode, _cases: &[WhenCase], _default: &Option<Vec<AstNode>>) {}
