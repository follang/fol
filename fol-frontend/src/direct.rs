...
pub enum DirectCompileMode {
    ...
    Check,
    Build { keep_build_dir: bool },
    Run { keep_build_dir: bool, args: Vec<String> },
    EmitRust { keep_build_dir: bool },
    EmitLowered,
}
...
pub fn run_direct_compile(
    ...
    match &config.mode {
        DirectCompileMode::Auto {
            ...
            let mut result = FrontendCommandResult::new("compile", format!("compiled {}", config.input));
            if *dump_lowered {
                let lowered_root = frontend_config.working_directory.join("target").join("lowered");
                std::fs::create_dir_all(&lowered_root).map_err(|error| {
                    FrontendError::new(
                        FrontendErrorKind::CommandFailed,
                        format!("failed to create lowered output root '{}': {error}", lowered_root.display()),
                    )
                })?;
                ...
                std::fs::write(&snapshot_path, render_lowered_workspace(&lowered)).map_err(|error| {
                    FrontendError::new(
                        FrontendErrorKind::CommandFailed,
                        format!("failed to write lowered snapshot '{}': {error}", snapshot_path.display()),
                    )
                })?;
        ...
        | DirectCompileMode::EmitRust { keep_build_dir } => {
            ...
            match (&config.mode, artifact) {
                (DirectCompileMode::EmitRust { .. }, fol_backend::BackendArtifact::RustSourceCrate { root, .. }) => {
                    let mut result = FrontendCommandResult::new(
                        "emit rust",
                ...
                (
                    ...
                    fol_backend::BackendArtifact::CompiledBinary { crate_root, binary_path },
                ) => {
                    let mut result = FrontendCommandResult::new(
                        "build",
...
pub fn run_direct_compile_with_io(
    ...
    match compile_file(&config.input, &resolver_config, &mut diagnostics) {
        Ok(lowered) => {
            ...
            if !diagnostics.has_errors() {
                if lowered.entry_candidates().is_empty()
                    && !matches!(config.mode, DirectCompileMode::Check | DirectCompileMode::EmitLowered)
                {
                    ...
                    match emit_backend_artifact(
                        ...
                        &BackendConfig {
                            mode: match config.mode {
                                DirectCompileMode::Auto { emit_rust: true, .. } => BackendMode::EmitSource,
                                DirectCompileMode::EmitRust { .. } => BackendMode::EmitSource,
                                _ => BackendMode::BuildArtifact,
                            },
...
fn compile_file(
    ...
    match ast_parser.parse_package(&mut lexer) {
        Ok(package) => {
            ...
            match fol_resolver::resolve_prepared_workspace_with_config(prepared, resolver_config.clone()) {
...
fn lower_compiler_glitch(error: &dyn Glitch) -> Option<fol_diagnostics::Diagnostic> {
    error
        ...
        .or_else(|| error.as_any().downcast_ref::<PackageError>().map(ToDiagnostic::to_diagnostic))
        .or_else(|| error.as_any().downcast_ref::<ResolverError>().map(ToDiagnostic::to_diagnostic))
        .or_else(|| error.as_any().downcast_ref::<TypecheckError>().map(ToDiagnostic::to_diagnostic))
        .or_else(|| error.as_any().downcast_ref::<LoweringError>().map(ToDiagnostic::to_diagnostic))
}
