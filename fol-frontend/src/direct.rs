use crate::{
    ...
    FrontendError, FrontendErrorKind, FrontendResult,
};
...
pub fn run_direct_compile(
    ...
    .map_err(|()| {
        FrontendError::new(
            ...
            render_direct_diagnostics(&diagnostics, config.json),
        )
    })?;
...
pub fn run_direct_compile_with_io(
    ...
    match compile_file(&config.input, &resolver_config, &mut diagnostics) {
        Ok(lowered) => {
            if config.dump_lowered && !config.json && !diagnostics.has_errors() {
...
fn render_direct_diagnostics(report: &DiagnosticReport, json: bool) -> String {
    report.output(if json {
        OutputFormat::Json
    } else {
        OutputFormat::Human
    })
