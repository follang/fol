...
use fol_stream::{FileStream, Source, SourceType, CharacterProvider};
use std::fs;
...
mod mod_handling_tests {
    ...
    fn test_mod_directory_skipping() {
        ...
        let sources = Source::init("test_old/main", SourceType::Folder)
            .expect("Should process test_old/main directory");
        ...
        for source in &sources {
            assert!(!source.path.contains(".mod/"), 
                   "Should not include files from .mod directories: {}", source.path);
        }
        ...
        let has_subdir_files = sources.iter().any(|s| s.path.contains("single/") || s.path.contains("var/"));
        assert!(has_subdir_files, "Should find files from non-.mod subdirectories");
    }

    #[test]
    fn test_folder_stream_creation() {
        // Test creating a stream from a folder with .mod directories
        let stream = FileStream::from_folder("test_old/main")
        ...
        for source in sources {
            assert!(!source.data.is_empty(), "Source should have data loaded: {}", source.path);
    ...
    fn test_multi_source_character_streaming() {
        ...
        let mut stream = FileStream::from_folder("test_old/main")
        ...
        println!("Streamed {} characters across {} file switches", char_count, file_switches);
    }
    ...
    fn test_mod_directory_contents_verification() {
        ...
        let main_mod_path = "test_old/main/main.mod";
        assert!(std::path::Path::new(main_mod_path).exists(), 
        ...
        assert!(has_other, "main.mod should contain non-.fol files (mixed content)");
    }
    ...
    fn test_source_path_methods() {
        ...
        assert!(abs_path.contains("main.fol"), "Absolute path should contain filename");
    ...
    fn test_sophisticated_stream_features() {
        ...
        for source in all_sources {
            assert!(source.path.ends_with(".fol"), 
                   "All sources should be .fol files: {}", source.path);
        }
    }
}