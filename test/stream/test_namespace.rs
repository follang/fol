...
mod namespace_tests {
    ...
    fn test_package_name_detection() {
        ...
        assert_eq!(package_name, "fol", "Package name should be 'fol' from Cargo.toml");
        
        for source in &sources {
            assert_eq!(source.package, *package_name, "All sources should have same package name");
        }
    ...
    fn test_root_namespace() {
        ...
        let sources = Source::init("test_old/main/main.fol", SourceType::File)
        ...
        assert_eq!(source.namespace, "fol", "Root file should have root namespace");
    ...
    fn test_subdirectory_namespace() {
        ...
        let subdir_sources: Vec<_> = sources.iter()
            .filter(|s| s.namespace != "fol") // Not root namespace
            .collect();
        
        assert!(!subdir_sources.is_empty(), "Should have sources in subdirectories");
        
        for source in &subdir_sources {
            println!("File: {} -> Namespace: {}", source.path, source.namespace);
            
            // Namespace should start with package name
            assert!(source.namespace.starts_with("fol::"), 
                   "Namespace should start with package name: {}", source.namespace);
            
            // Should contain directory structure
            if source.path.contains("/var/") {
                assert!(source.namespace.contains("var"), 
                       "Should contain 'var' in namespace: {}", source.namespace);
            }
            
            if source.path.contains("/single/") {
                assert!(source.namespace.contains("single"), 
                       "Should contain 'single' in namespace: {}", source.namespace);
            }
        }
    }
    ...
    fn test_namespace_structure_mapping() {
        ...
        if !std::path::Path::new(test_dir).exists() {
            ...
            fs::write(&format!("{}/math/vector/vec3.fol", test_dir), "// Vector3 implementation").ok();
            fs::write(&format!("{}/utils/helper.fol", test_dir), "// Utility functions").ok();
        }
        ...
        for source in &sources {
            let filename = std::path::Path::new(&source.path).file_name()
                .and_then(|n| n.to_str()).unwrap_or("");
            
            match filename {
                "main.fol" => {
                    assert_eq!(source.namespace, "myproject", 
                              "Root file should have root namespace");
                }
                "mod.fol" => {
                    assert_eq!(source.namespace, "myproject::math", 
                              "Math module should be in math namespace");
                }
                "vec3.fol" => {
                    assert_eq!(source.namespace, "myproject::math::vector", 
                              "Vector file should be in math::vector namespace");
                }
                "helper.fol" => {
                    assert_eq!(source.namespace, "myproject::utils", 
    ...
    fn test_mod_directories_excluded_from_namespace() {
        ...
        for source in &sources {
            assert!(!source.namespace.contains(".mod"), 
                   "Namespace should not contain .mod: {}", source.namespace);
            
            // But the files from .mod directories should not be present at all
            assert!(!source.path.contains(".mod/"), 
                   "Should not have files from .mod directories: {}", source.path);
        }
    ...
    fn test_namespace_consistency() {
        ...
        for source in &sources {
            ...
            if let Some(existing_namespace) = dir_to_namespace.get(&dir) {
                assert_eq!(existing_namespace, &source.namespace,
    ...
    fn test_explicit_package_name() {
        ...
        let sources = Source::init_with_package("test_old/main/main.fol", SourceType::File, "custom_package")
            .expect("Should create source with custom package");
        
        assert_eq!(sources.len(), 1, "Should have one source");
        let source = &sources[0];
        
        assert_eq!(source.package, "custom_package", "Should use explicit package name");
        assert_eq!(source.namespace, "custom_package", "Should use custom package as root namespace");
    }
    ...
    fn test_namespace_output_integration() {
        ...
        for source in &sources {
            assert!(!source.namespace.is_empty(), "Namespace should not be empty");
            ...
            assert!(source.namespace.chars().all(|c| c.is_alphanumeric() || c == ':' || c == '_'),
                   "Namespace should only contain valid characters: {}", source.namespace);
            ...
            if source.namespace.contains("::") {
                ...
                assert!(parts.len() >= 2, "Namespaced item should have at least 2 parts");
                assert_eq!(parts[0], source.package, "First part should be package name");
            }
        }
