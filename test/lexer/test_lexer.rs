...
mod lexer_tests {
    ...
    fn test_keywords() {
        ...
        let keyword_strings: Vec<String> = keywords.iter().map(|(_, content)| content.clone()).collect();
        
        assert!(keyword_strings.contains(&"use".to_string()), "Should contain 'use' keyword");
        assert!(keyword_strings.contains(&"var".to_string()), "Should contain 'var' keyword");
        assert!(keyword_strings.contains(&"fun".to_string()), "Should contain 'fun' keyword");
        assert!(keyword_strings.contains(&"pro".to_string()), "Should contain 'pro' keyword");
        assert!(keyword_strings.contains(&"typ".to_string()), "Should contain 'typ' keyword");
    }
    ...
    fn test_literals() {
        ...
        // Filter out spaces and EOF
        let literals: Vec<_> = tokens.iter()
            .filter(|(key, _)| key.is_literal())
            .collect();
        
        assert!(literals.len() > 0, "Should have literal tokens");
        
        let literal_strings: Vec<String> = literals.iter().map(|(_, content)| content.clone()).collect();
        
        // Check for different types of literals
        let has_decimal = literal_strings.iter().any(|s| s.chars().all(|c| c.is_ascii_digit()));
        let has_float = literal_strings.iter().any(|s| s.contains('.'));
        let _has_string = literal_strings.iter().any(|s| s.starts_with('"') && s.ends_with('"'));
    ...
    fn test_symbols() {
        ...
        let symbol_strings: Vec<String> = symbols.iter().map(|(_, content)| content.clone()).collect();
        
        // Check for specific symbols - escape braces in format strings
        assert!(symbol_strings.contains(&"{".to_string()), "Should contain opening brace");
        assert!(symbol_strings.contains(&"}".to_string()), "Should contain closing brace");
        assert!(symbol_strings.contains(&"(".to_string()), "Should contain '('");
        assert!(symbol_strings.contains(&")".to_string()), "Should contain ')'");
        assert!(symbol_strings.contains(&";".to_string()), "Should contain ';'");
        assert!(symbol_strings.contains(&",".to_string()), "Should contain ','");
    ...
    fn test_identifiers() {
        ...
        let identifiers: Vec<_> = tokens.iter()
            .filter(|(key, _)| key.is_ident())
            .collect();
        
        assert!(identifiers.len() > 0, "Should have identifier tokens");
        
        let identifier_strings: Vec<String> = identifiers.iter().map(|(_, content)| content.clone()).collect();
        ...
        let _has_camel_case = identifier_strings.iter().any(|s| s.chars().any(|c| c.is_uppercase()));
    ...
    fn test_comments() {
        ...
        assert!(tokens.len() > 0, "Should successfully tokenize file with comments");
        
        // Check that we have some non-comment tokens (the actual code)
        let non_void_tokens: Vec<_> = tokens.iter()
            .filter(|(key, _)| !key.is_void() && !key.is_comment())
            .collect();
        
        assert!(non_void_tokens.len() > 0, "Should have actual code tokens besides comments");
        
        println!("Total tokens: {}, Comments: {}, Code tokens: {}", 
                 tokens.len(), comments.len(), non_void_tokens.len());
    }
    ...
    fn test_mixed_content() {
        ...
        // Filter out spaces
        let code_tokens: Vec<_> = tokens.iter()
            .filter(|(key, _)| !key.is_space() && !key.is_eof())
            .collect();
        
        assert!(code_tokens.len() > 10, "Should have substantial number of tokens");
        ...
        let has_symbols = code_tokens.iter().any(|(key, _)| matches!(key, KEYWORD::Symbol(_)));
    ...
    fn test_bracket_matching() {
        ...
        let brackets: Vec<_> = tokens.iter()
            .filter(|(key, _)| key.is_bracket())
            .collect();
        
        let open_brackets = brackets.iter().filter(|(key, _)| key.is_open_bracket()).count();
        let close_brackets = brackets.iter().filter(|(key, _)| key.is_close_bracket()).count();
        
        println!("Open brackets: {}, Close brackets: {}", open_brackets, close_brackets);
        assert_eq!(open_brackets, close_brackets, "Should have matching brackets");
    }

    #[test]
    fn test_token_position_tracking() {
        let mut file_stream = FileStream::from_file("test/lexer/mixed.fol")
            .expect("Should read mixed.fol");
        ...
        for i in 0..5 {
            match lexer.curr(false) {
                Ok(token) => {
                    ...
                    println!("Token {}: '{}' at row {}, col {}", 
                             i, token.con(), loc.row(), loc.col());
...
mod lexer_performance_tests {
    ...
    fn test_lexer_performance() {
        ...
        if std::path::Path::new(test_file).exists() {
            ...
            assert!(duration.as_millis() < 100, "Should tokenize reasonably quickly");
        }
    }
...
mod lexer_error_tests {
    ...
    fn test_nonexistent_file_error() {
        let result = std::panic::catch_unwind(|| {
            tokenize_file("test/lexer/nonexistent.fol")
        });
