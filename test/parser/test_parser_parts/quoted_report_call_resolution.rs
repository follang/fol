...
#[test]
fn test_function_custom_error_type_accepts_report_forward_overloaded_quoted_method_call_result_by_arity(
) {
    let mut file_stream = FileStream::from_file(
        "test/parser/simple_fun_error_type_report_forward_quoted_method_overload_arity_ok.fol",
    )
    .expect("Should read forward overloaded quoted report method-call result compatible fixture");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    parser.parse(&mut lexer).expect(
        "Parser should accept forward overloaded quoted report method call result when selected arity return type is compatible",
    );
}

#[test]
fn test_function_custom_error_type_rejects_report_forward_overloaded_quoted_method_call_result_by_arity_mismatch(
) {
    let mut file_stream = FileStream::from_file(
        "test/parser/simple_fun_error_type_report_forward_quoted_method_overload_arity_mismatch.fol",
    )
    .expect("Should read forward overloaded quoted report method-call result mismatch fixture");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let errors = parser.parse(&mut lexer).expect_err(
        "Parser should reject forward overloaded quoted report method call result when selected arity return type is incompatible",
    );
    ...
    assert!(
        parse_error.to_string().contains("Reported expression type")
            && parse_error
                .to_string()
                .contains("incompatible with routine error type"),
        "Forward overloaded quoted method-call result mismatch should report incompatible expression type, got: {}",
        parse_error
    );
}

#[test]
fn test_procedure_custom_error_type_accepts_report_forward_overloaded_quoted_method_call_result_by_arity(
) {
    let mut file_stream = FileStream::from_file(
        "test/parser/simple_pro_error_type_report_forward_quoted_method_overload_arity_ok.fol",
    )
    .expect(
        "Should read forward overloaded quoted procedure report method-call result compatible fixture",
    );

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    parser.parse(&mut lexer).expect(
        "Parser should accept forward overloaded quoted procedure report method call result when selected arity return type is compatible",
    );
}

#[test]
fn test_procedure_custom_error_type_rejects_report_forward_overloaded_quoted_method_call_result_by_arity_mismatch(
) {
    let mut file_stream = FileStream::from_file(
        "test/parser/simple_pro_error_type_report_forward_quoted_method_overload_arity_mismatch.fol",
    )
    .expect(
        "Should read forward overloaded quoted procedure report method-call result mismatch fixture",
    );

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let errors = parser.parse(&mut lexer).expect_err(
        "Parser should reject forward overloaded quoted procedure report method call result when selected arity return type is incompatible",
    );
    ...
    assert!(
        parse_error.to_string().contains("Reported expression type")
            && parse_error
                .to_string()
                .contains("incompatible with routine error type"),
        "Forward overloaded quoted procedure method-call result mismatch should report incompatible expression type, got: {}",
        parse_error
    );
}

#[test]
fn test_function_custom_error_type_accepts_report_forward_overloaded_keyword_method_call_result_by_arity(
) {
    let mut file_stream = FileStream::from_file(
        "test/parser/simple_fun_error_type_report_forward_keyword_method_overload_arity_ok.fol",
    )
    .expect(
        "Should read forward overloaded keyword-named report method-call result compatible fixture",
    );

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    parser.parse(&mut lexer).expect(
        "Parser should accept forward overloaded keyword-named report method call result when selected arity return type is compatible",
    );
}

#[test]
fn test_function_custom_error_type_rejects_report_forward_overloaded_keyword_method_call_result_by_arity_mismatch(
) {
    let mut file_stream = FileStream::from_file(
        "test/parser/simple_fun_error_type_report_forward_keyword_method_overload_arity_mismatch.fol",
    )
    .expect(
        "Should read forward overloaded keyword-named report method-call result mismatch fixture",
    );

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let errors = parser.parse(&mut lexer).expect_err(
        "Parser should reject forward overloaded keyword-named report method call result when selected arity return type is incompatible",
    );
    ...
    assert!(
        parse_error.to_string().contains("Reported expression type")
            && parse_error
                .to_string()
                .contains("incompatible with routine error type"),
        "Forward overloaded keyword-named method-call result mismatch should report incompatible expression type, got: {}",
        parse_error
    );
}

#[test]
fn test_procedure_custom_error_type_accepts_report_forward_overloaded_keyword_method_call_result_by_arity(
) {
    let mut file_stream = FileStream::from_file(
        "test/parser/simple_pro_error_type_report_forward_keyword_method_overload_arity_ok.fol",
    )
    .expect(
        "Should read forward overloaded keyword-named procedure report method-call result compatible fixture",
    );

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    parser.parse(&mut lexer).expect(
        "Parser should accept forward overloaded keyword-named procedure report method call result when selected arity return type is compatible",
    );
}

#[test]
fn test_procedure_custom_error_type_rejects_report_forward_overloaded_keyword_method_call_result_by_arity_mismatch(
) {
    let mut file_stream = FileStream::from_file(
        "test/parser/simple_pro_error_type_report_forward_keyword_method_overload_arity_mismatch.fol",
    )
    .expect(
        "Should read forward overloaded keyword-named procedure report method-call result mismatch fixture",
    );

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let errors = parser.parse(&mut lexer).expect_err(
        "Parser should reject forward overloaded keyword-named procedure report method call result when selected arity return type is incompatible",
    );
    ...
    assert!(
        parse_error.to_string().contains("Reported expression type")
            && parse_error
                .to_string()
                .contains("incompatible with routine error type"),
        "Forward overloaded keyword-named procedure method-call result mismatch should report incompatible expression type, got: {}",
        parse_error
    );
}
