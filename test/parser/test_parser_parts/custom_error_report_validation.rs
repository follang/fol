...
fn test_function_custom_error_type_rejects_incompatible_report_identifier() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_fun_error_type_report_identifier_mismatch.fol")
...
fn test_function_custom_error_type_rejects_incompatible_report_expression() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_fun_error_type_report_expression_mismatch.fol")
    ...
    let errors = parser
        .parse(&mut lexer)
        .expect_err("Parser should reject report expression incompatible with custom error type");
...
fn test_function_custom_error_type_rejects_report_local_inferred_from_expression_mismatch() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_fun_error_type_report_expr_local_mismatch.fol")
...
#[test]
fn test_function_custom_error_type_accepts_nested_block_report_local() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_fun_error_type_report_nested_block_ok.fol")
            .expect("Should read nested-block local report compatible file");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    parser
        .parse(&mut lexer)
        .expect("Parser should accept compatible report local inside nested block");
}

#[test]
fn test_function_custom_error_type_rejects_nested_block_report_local_mismatch() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_fun_error_type_report_nested_block_mismatch.fol")
            .expect("Should read nested-block local report mismatch file");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let errors = parser
        .parse(&mut lexer)
        .expect_err("Parser should reject incompatible report local inside nested block");
    ...
    assert!(
        first_message.contains("Reported identifier")
            && first_message.contains("incompatible with routine error type"),
        "Nested-block report mismatch should report incompatible identifier type, got: {}",
        first_message
    );
}
...
fn test_function_custom_error_type_rejects_unknown_report_identifier() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_fun_error_type_report_unknown_identifier.fol")
...
#[test]
fn test_function_custom_error_type_rejects_report_container_expression() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_fun_error_type_report_container_mismatch.fol")
            .expect("Should read report container mismatch fixture");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let errors = parser.parse(&mut lexer).expect_err(
        "Parser should reject report container expression incompatible with custom error type",
    );
    ...
    assert!(
        first_message.contains("Reported expression type 'container'")
            && first_message.contains("incompatible with routine error type"),
        "Container report mismatch should report incompatible expression type, got: {}",
        first_message
    );
}

#[test]
fn test_function_custom_error_type_rejects_report_range_expression() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_fun_error_type_report_range_mismatch.fol")
            .expect("Should read report range mismatch fixture");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let errors = parser
        .parse(&mut lexer)
        .expect_err("Parser should reject report range incompatible with custom error type");
    ...
    assert!(
        first_message.contains("Reported expression type 'range'")
            && first_message.contains("incompatible with routine error type"),
        "Range report mismatch should report incompatible expression type, got: {}",
        first_message
    );
}
...
fn test_procedure_custom_error_type_rejects_incompatible_report_identifier() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_pro_error_type_report_identifier_mismatch.fol")
...
fn test_procedure_custom_error_type_rejects_incompatible_report_expression() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_pro_error_type_report_expression_mismatch.fol")
...
#[test]
fn test_procedure_custom_error_type_accepts_nested_block_report_local() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_pro_error_type_report_nested_block_ok.fol")
            .expect("Should read nested-block procedure local report compatible file");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    parser
        .parse(&mut lexer)
        .expect("Parser should accept compatible procedure report local inside nested block");
}

#[test]
fn test_procedure_custom_error_type_rejects_nested_block_report_local_mismatch() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_pro_error_type_report_nested_block_mismatch.fol")
            .expect("Should read nested-block procedure local report mismatch file");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let errors = parser
        .parse(&mut lexer)
        .expect_err("Parser should reject incompatible procedure report local inside nested block");
    ...
    assert!(
            first_message.contains("Reported identifier")
                && first_message.contains("incompatible with routine error type"),
            "Nested-block procedure report mismatch should report incompatible identifier type, got: {}",
            first_message
        );
}

#[test]
fn test_procedure_custom_error_type_rejects_unknown_identifier_inside_report_container() {
    let mut file_stream = FileStream::from_file(
        "test/parser/simple_pro_error_type_report_container_unknown_identifier.fol",
    )
    .expect("Should read unknown procedure report identifier container fixture");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let errors = parser.parse(&mut lexer).expect_err(
            "Parser should reject unknown identifier inside procedure report container in custom-error routine",
        );
    ...
    assert!(
            first_message.contains("Unknown reported identifier 'missing_err'"),
            "Unknown identifier inside procedure report container should produce explicit diagnostic, got: {}",
            first_message
        );
}

#[test]
fn test_procedure_custom_error_type_rejects_unknown_identifier_inside_report_range() {
    let mut file_stream = FileStream::from_file(
        "test/parser/simple_pro_error_type_report_range_unknown_identifier.fol",
    )
    .expect("Should read unknown procedure report identifier range fixture");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let errors = parser.parse(&mut lexer).expect_err(
            "Parser should reject unknown identifier inside procedure report range in custom-error routine",
        );
    ...
    assert!(
            first_message.contains("Unknown reported identifier 'missing_err'"),
            "Unknown identifier inside procedure report range should produce explicit diagnostic, got: {}",
            first_message
        );
}
...
fn test_procedure_custom_error_type_rejects_report_container_expression() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_pro_error_type_report_container_mismatch.fol")
...
fn test_procedure_custom_error_type_rejects_report_range_expression() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_pro_error_type_report_range_mismatch.fol")
...
fn test_function_custom_error_type_rejects_report_call_result_incompatible_with_error_type() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_fun_error_type_report_call_result_mismatch.fol")
    ...
    let errors = parser
        .parse(&mut lexer)
        .expect_err("Parser should reject report call result incompatible with custom error type");
...
fn test_procedure_custom_error_type_rejects_report_call_result_incompatible_with_error_type() {
    let mut file_stream =
        FileStream::from_file("test/parser/simple_pro_error_type_report_call_result_mismatch.fol")
