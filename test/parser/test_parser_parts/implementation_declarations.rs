...
#[test]
fn test_implementation_declaration_supports_named_and_quoted_constraints() {
    let mut file_stream = FileStream::from_file("test/parser/simple_named_generic_constraints.fol")
        .expect("Should read named generic-constraint fixture");

    let mut lexer = Elements::init(&mut file_stream);
    let mut parser = AstParser::new();
    let ast = parser
        .parse(&mut lexer)
        .expect("Parser should parse named and quoted implementation constraints");
